 ...
 */
app.get('/api/pedidos/activo', async (req, res) => {
    ...
    try {
        ...
        const result = await (0, db_1.query)(`SELECT id_pedido, nombre_cliente, telefono, direccion, referencia_lugar, descripcion_pedido, estatus 
       FROM pedido 
       WHERE id_repartidor = $1 AND estatus IN (1, 2, 3, 5) 
       ORDER BY id_pedido DESC 
       LIMIT 1`, [parsedRepartidorId]);
 ...
 */
app.patch('/api/pedidos/:id/estatus', async (req, res) => {
    ...
    try {
        ...
        const orderCheck = await (0, db_1.query)('SELECT id_pedido, id_repartidor FROM pedido WHERE id_pedido = $1', [orderId]);
        ...
        const updateResult = await (0, db_1.query)('UPDATE pedido SET estatus = $1 WHERE id_pedido = $2 RETURNING *', [estatus, orderId]);
        // 2. Insert into historialestatus
        await (0, db_1.query)(`INSERT INTO historialestatus (estatus, id_usuario, id_pedido) 
       VALUES ($1, $2, $3)`, [statusStr, repartidorId, orderId]);
